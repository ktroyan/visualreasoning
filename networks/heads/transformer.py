...
def create_absolute_positional_encoding(ape_type: str, ape_length: int, embed_dim: int) -> nn.Parameter:
    ...
    NOTE: The BOS token is considered for the APE, and the current token to predict is not, hence the APE length is max_seq_len.
    """
...
class TransformerDecoder(nn.TransformerDecoder):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None, memory_key_padding_mask=None):
        ...
        # NOTE: We make the choice to output the embeddings and not output the logits ([B, tgt_seq_len, num_classes]) by not using an additional Linear layer here
