...
class REARCModel(VisReasModel):
    ...
    def forward(self, x, y, samples_task_id=None, x_grid_object_ids=None):
        ...
        if self.model_config.ope.enabled and (x_grid_object_ids is not None) and self.model_config.backbone in ["vit", "looped_vit", "transformer"]:
            ...
            x_encoded = self.encoder(x, x_grid_object_ids)  # [B, seq_len, bb_embed_dim]; NOTE: the extra tokens will have been truncated so the encoded sequence will also have a dim seq_len 
