...
def get_pso_difficulty(
    ...
    obj_dens = (len(objects(i, T, F, F)) / hwi +
                len(objects(o, T, F, F)) / hwo) / 2
...
# NOTE: I modified the code in order to accomodate our needs regarding the datasets generation
def generate_dataset(
    ...
    if len(specific_tasks) > 0:
        keys = specific_tasks
    # Randomly select n_tasks tasks
    if n_tasks != -1:
        random.shuffle(keys)    # randomly shuffle the tasks
        keys = keys[:n_tasks]
...
def demo_dataset(
    ...
    for i, fn in enumerate(sorted(os.listdir(f'{folder}/tasks'))):
        if s <= i < e:
            ...
            original_task = [format_example(
                example) for example in original_task['train'] + original_task['test']]
            generated_task = [format_example(example)
                              for example in generated_task[:10*n]]
            difficulties = metadata[key]['pso_difficulties'][:9*n]
            generated_task = [ex for ex, diff in sorted(
                zip(generated_task, difficulties), key=lambda item: item[1])]
