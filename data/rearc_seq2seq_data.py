...
class REARCSeq2SeqDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # TODO: see how to handle the transformation (different from the Vision approach?)
        # if self.transform is not None:
        #     x = self.transform(x)

        # Tokenize the input tensor in a sequence to create a sequence to be given to a Transformer, instead of an image to be given to a ViT 
        x = self.tokenize_tensor(x)

        return x, y, sample_task_id


class REARCSeq2SeqDataModule(DataModuleBase):
    ...
    def __init__(self, data_config, image_size=30, **kwargs):
        ...
        self.train_set = REARCSeq2SeqDataset(train_set_path, image_size=image_size, transform=transform)
        self.val_set = REARCSeq2SeqDataset(val_set_path, image_size=image_size, transform=transform)
        self.test_set = REARCSeq2SeqDataset(test_set_path, image_size=image_size, transform=transform)

        if data_config.use_gen_test_set:
            gen_test_set_path = data_config.dataset_dir + '/test_gen.json'
            self.gen_test_set = REARCSeq2SeqDataset(gen_test_set_path, image_size=image_size, transform=transform)
